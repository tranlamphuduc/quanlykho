...
@section('header')
<i class="bi bi-pencil me-2"></i>Sửa phiếu nhập kho: {{ $stockIn->code }}
@endsection
        ...
        <form action="{{ route('stock-in.update', $stockIn) }}" method="POST">
            ...
            <div class="row mb-3">
                <div class="col-md-3 mb-2">
                    ...
                    <input type="text" class="form-control" value="{{ $stockIn->code }}" readonly>
                </div>
                <div class="col-md-3 mb-2">
                    <label class="form-label">Kho nhập *</label>
                    <select name="warehouse_id" class="form-select" required>
                        @foreach($warehouses as $wh)
                        <option value="{{ $wh->id }}" {{ $stockIn->warehouse_id == $wh->id ? 'selected' : '' }}>{{ $wh->name }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="col-md-3 mb-2">
                    ...
                    <select name="supplier_id" class="form-select">
                        ...
                        <option value="{{ $sup->id }}" {{ $stockIn->supplier_id == $sup->id ? 'selected' : '' }}>{{ $sup->name }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="col-md-3 mb-2">
                    <label class="form-label">Ghi chú</label>
                    <input type="text" name="note" class="form-control" value="{{ $stockIn->note }}">
                </div>
            </div>
                    ...
                    <tr>
                        <td>
                            <select name="product_id[]" class="form-select form-select-sm" onchange="fillPrice(this)" required>
                                ...
                                <option value="{{ $p->id }}" data-price="{{ $p->cost_price }}" {{ $detail->product_id == $p->id ? 'selected' : '' }}>{{ $p->code }} - {{ $p->name }}</option>
                                @endforeach
                            </select>
                        </td>
            ...
            <div class="d-flex gap-2">
                <a href="{{ route('stock-in.index') }}" class="btn btn-secondary">Quay lại</a>
